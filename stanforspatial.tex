...
Define the model in a \verb0.stan0 file, e.g. a regression model:
\begin{verbatim}
data {
  int<lower = 1> n_obs;
  int<lower = 1> n_cov;
...
\ \ \ \ \ \ \ \, and `\verb0//0' comments the rest of the line.
\end{frame}
...
\begin{itemize}
\item Stan can hand simple constaints stated in terms of lower \& upper bounds, e.g. \verb0real<lower = mu_x> mu_y;0,
...
\end{verbatim}
Each sampling statement (`\verb0~0') adds the relevant quantity to the target log-density. The LHS must be a previously defined variable (data, parameter, or transformed parameter blocks).\\~\\
`\verb0*0' is matrix multiplication.\\~\\
...
Required arguments:
\begin{itemize}
\item A model --- ``.stan'' file, or a previous \verb0stan0 object.
\item Data --- list of all variables in the data block of the model.\\~\\
\end{itemize}
Useful named arguments and their defaults:
\begin{itemize}
\item cores = 1 --- number of cores to use (uses \verb0parallel0 backend).
\item chains = 4 --- number of chains.
\item iter = 4000 --- total number of iterations per chain.
\item warmup = iter/2 --- number of iterations to use for tuning / burn-in.\\~\\
\end{itemize}
