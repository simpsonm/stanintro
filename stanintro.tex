...
\item Under easy conditions $k(\cdot|\cdot)$ is {\color{red}ergodic}, $\bm{q}^{(t)} \to {\color{blue}\pi(\bm{q})}$, and
\begin{align*}
...
\item Additionally if $k(\cdot|\cdot)$ is {\color{red}geometric ergodic}:
\begin{align*}
