...
Let $s[i]$ denote the state that county $i$ is in, and $\alpha_s$ denote the intercept for state $s$.\\~\\
Now we specify the model directly on centered and scaled $y_i$ and $\bm{x}_i'$:
\begin{align*}
y_i | \bm{\alpha} \stackrel{ind}{\sim} \mathrm{N}(\alpha_{s[i]} + \bm{x}_i'\bm{\beta}, \sigma^2), && \alpha_s \stackrel{iid}{\sim} \mathrm{N}(\mu_{\alpha}, \sigma_{\alpha}^2),
\end{align*}
for $i=1,2,\dots,N$, $s=1,2,\dots,S$.
\end{frame}

\begin{frame}[fragile]
\frametitle{Hierarhical Model in Stan}
From \verb0rand_intercept_reg.stan0:
\begin{verbatim}
parameters {
  vector[n_cov] beta;
  real<lower = 0> sigma;
...
\begin{frame}
\frametitle{Slides Left:}
\begin{enumerate}
\item Results of hierarchical model
\begin{enumerate}
\item divergent transitions
\item exceptions
\end{enumerate}
\item Noncentered hierarchical model
\item results of noncenetered model
\item when is stan good (sorts of models)
\begin{enumerate}
\item cheap likelihoods
\item not-too-large covariance matrices
\item not-too-large mixtures
\end{enumerate}
\end{enumerate}
\end{frame}
\end{document}
